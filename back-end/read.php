...
function login($email, $senha){
    if(verificarCredenciaisNoBanco($email, $senha)){
        // Se as credenciais estiverem corretas, busca o usuário e redireciona para a página de teste
        $usuario = buscarUsuarioPorEmail($email);
        if($usuario) {
            echo "<script>window.alert('Login realizado com sucesso!');</script>";
            echo "<h1>Dados do usuário</h1>";
            echo "ID: " . $usuario->getUsuarioID() . "<br>";
            ...
            echo "<script>window.alert('Erro ao buscar usuário!');</script>";
